...
<script>
    ...
    document.getElementById("submitButton").addEventListener("click", async function() {
        ...
        try {
            initializeApiBaseUrl();
            let response = await fetch(`${API_BASE_URL}api/Uploading/excel-upload/offeredcourse_teacherallocation`, {
                method: "POST"
                , body: formData
            , });
            ...
            if (response.status === 200) {
                ...
                showAlert(`Upload Successful!\nSuccessfully Added Records Count: ${result.data["Total Inserted"]}\nFaulty Records Count: ${result.data["Total Failure"]}`, 'success');
            } else {
                hideLoader();
    ...
    function renderTimetable(response) {
        let success = response.data['Sucessfull']; // Typo in API response (should be "Success")
        let error = response.data['FaultyData'];

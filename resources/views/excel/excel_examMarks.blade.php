...
<head>
    ...
    <title>LMS - Exam Marks List</title>
...
<body class="bg-gradient-to-r from-blue-300 via-blue-200 to-blue-100 min-h-screen p-0 m-0">
    ...
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6 mt-6 text-center">
        ...
        <h2 class="text-2xl font-bold text-gray-700 mb-4">Upload Exam Marks List For A Section</h2>
        <div class="flex justify-center space-x-4 ">
            <label for="sessionDropdown" class="block text-gray-700 font-bold mb-2">Select Course:</label>
        ...
        <br>
        <div class="flex justify-center space-x-4">
            <label for="examTypeDropdown" class="block text-gray-700 font-bold mb-2">
                Select Exam Type:
            </label>
            <select id="examTypeDropdown" class="block w-64 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
...
<script>
    ...
    document.getElementById("submitButton").addEventListener("click", async function() {
        ...
        let type = document.getElementById("examTypeDropdown").value;
        ...
        try {
            initializeApiBaseUrl();
            let response = await fetch(`${API_BASE_URL}api/Uploading/uplaod/Result`, {
                method: "POST"
                , body: formData
            , });
    ...
    function renderTimetable(response) {
        let success = response['Succes']; // Typo in API response (should be "Success")
                        ...
                        <tbody>
                            ${success.map(record => `
                                <tr class="bg-green-100">
                                    ...
                                    <td class="border p-2 text-center">${record.Added??'N.A'}</td>
                    ...
                    <table class="w-full border-collapse border border-gray-300 text-gray-700">
                        ...
                        <tbody>
                            ${error.map(record => `
                                <tr class="bg-red-100">
                                    <td class="border p-2 text-center">${record.status}</td>
                                    <td class="border p-2 text-center">${record.issue??'N/A'}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
